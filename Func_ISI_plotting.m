...
% file_location='G:\[JH4209]\[JH][Recording]\REC_2013-03-06\Data_3Ch_[2013-03-06_21-47-38]_TOP0_180degHz.mat'
% figH=figure(1);
...
%% Data process (average firing rate)
...
i=2;
while(SamplingRate*TotalPeriod +1 - i )
    if (xor((data(3,i)>=SM_dir_threshold) , (data(3,i-1)>=SM_dir_threshold))) %... rising edge

%     end
%     if ((data(3,i)<=SM_dir_threshold) && (data(3,i-1)>=SM_dir_threshold)) %... fallng edge
...
plot(t,avrage_spike_rate(1,:),'y') %avrage_spike_rate
hold off
axis([0 TotalPeriod 0 400]);
xlabel('Time(sec)')
